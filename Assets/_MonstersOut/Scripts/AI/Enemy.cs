...
/* 
 * This is the base enemy script, control some important enemy in the game
 */
...
namespace RGame
{
    ...
    public class Enemy : MonoBehaviour, ICanTakeDamage, IListener
    {
        ...
        [Tooltip("set the start action for enemy")]
        ...
        {
            //get the facing of the enemy by the rotation
        ...
        {
            //get the component and init the health
            ...
            //init the move speed and the world patrol poinnt
            moveSpeed = walkSpeed;
            ...
            //Check and call the start behavior action
        ...
        public virtual void DetectPlayer(float delayChase = 0)
        {
            if (isPlayerDetected)
                return;
            //Chase the player after the delay time
            StartCoroutine(DelayBeforeChasePlayer(delayChase));
        }

        //Delay a time before chasing the player
        ...
        public virtual void Hit(Vector2 force = default(Vector2), bool pushBack = false, bool knockDownRagdoll = false, bool shock = false)
        {
            ...
            //Check the hit behavior to decide chase it or just standing
            switch (hitBehavior)
            {
        ...
        public virtual void Die()
        {
            ...
            //If die while frozing, spawn the frozen effect
            ...
            if (enemyEffect == ENEMYEFFECT.EXPLOSION)
            {
                if (bloodPuddleFX)
                {
                    ...
                    {
                        Instantiate(bloodPuddleFX, (Vector2)transform.position + new Vector2(Random.Range(-(randomBloodPuddlePoint.x * 2), randomBloodPuddlePoint.x * 2), Random.Range(-(2 * randomBloodPuddlePoint.y), 2 * randomBloodPuddlePoint.y)), Quaternion.identity);
                    }
                }
        ...
        public void Heal(int value)
        {
            ...
            if (healthBar)
                healthBar.UpdateValue(currentHealth / (float)health);
            FloatingTextManager.Instance.ShowText("" + (int)value, healthBarOffset, Color.blue, transform.position);
        }
        ...
        public void TakeDamage(float damage, Vector2 force, Vector2 hitPoint, GameObject instigator, BODYPART bodyPart = BODYPART.NONE, WeaponEffect weaponEffect = null)
        {
            ...
            _damage = damage;
            //get the hit point to show the effect at this
            hitPos = hitPoint;
            bool isExplosion = false;
            //caculating the health
            currentHealth -= (int)damage;
            FloatingTextManager.Instance.ShowText("" + (int)damage, healthBarOffset, Color.red, transform.position);

            knockBackForce = force;
            //spawn the hit effect at the random position
            if (hitFX)
                Instantiate(hitFX, hitPos + new Vector2(Random.Range(-randomHitPoint.x, randomHitPoint.x), Random.Range(-randomHitPoint.y, randomHitPoint.y)), Quaternion.identity);
            if (bloodPuddleFX)
                Instantiate(bloodPuddleFX, (Vector2)transform.position + new Vector2(Random.Range(-randomBloodPuddlePoint.x, randomBloodPuddlePoint.x), Random.Range(-randomBloodPuddlePoint.y, randomBloodPuddlePoint.y)), Quaternion.identity);
            ...
            {
                //Only check if there are the weapon effect
                if (weaponEffect != null)
                {
                    switch (weaponEffect.effectType)
                    {
                        case WEAPON_EFFECT.POISON:
                            //make the enemy poison
